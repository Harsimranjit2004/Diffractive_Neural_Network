...
LAYER_DISTANCE = 20e-3 # Reduced to 20 mm for tighter diffraction
LAYER_SIZE = 200       # 200x200 pixels
NUM_LAYERS = 5         # 5 diffractive layers

def detector_region(x):
    # Circular detector layout (50x50 pixels, 10 regions)
    batch_size = x.shape[0]
    outputs = torch.zeros(batch_size, 10, device=device)
    center = LAYER_SIZE // 2
    radius = LAYER_SIZE // 3  # Increased radius for better separation
    detector_size = 50
    for i in range(10):
        angle = 2 * np.pi * i / 10
...
class DiffractiveLayer(nn.Module):
    def __init__(self, layer_size=200, wavelength=750e-6, pixel_size=400e-6, distance=20e-3):
        ...
        kz_squared = torch.clamp(kz_squared, min=0)  # Prevent negative sqrt
    ...
    def forward(self, waves):
        ...
        fft = torch.fft.fft2(waves_complex)  # (batch, 200, 200)
        # print(f"fft shape={fft.shape}, H shape={self.H.shape}")  # Debug
        fft = fft * self.H
...
def visualize_fields(model, images, labels, class_names):
    model.eval()
    with torch.no_grad():
    ...
    for i in range(num_images):
        ...
        axes[i, 0].axis('off')
        axes[i, 1].imshow(x_abs[i].cpu(), cmap='inferno')
        ...
        axes[i, 1].set_title(f'Output Intensity')
        axes[i, 1].axis('off')
    
    normalized_outputs = outputs / (torch.sum(outputs, dim=1, keepdim=True) + 1e-6)
    print(f"Sample outputs (normalized):\n{normalized_outputs.cpu().numpy()[:5]}")
    print(f"Predicted classes: {torch.argmax(outputs, dim=1)[:5].cpu().numpy()}")
...
def main(args):
    ...
    for epoch in range(args.start_epoch + 1, args.start_epoch + 1 + args.num_epochs):
        ...
        # Visualize fields every 5 epochs
        if epoch % 5 == 0:
            ...
            fig = visualize_fields(model, sample_images, sample_labels, class_names)
