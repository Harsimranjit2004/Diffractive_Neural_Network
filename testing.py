...
LAYER_DISTANCE = 30e-3 # 30 mm between layers
...
class PhaseMask(nn.Module):
    def __init__(self, size):
        ...
        nn.init.uniform_(self.phase, a=0, b=2 * np.pi)  # 0 to 2π per paper
...
class AngularSpectrumPropagation(nn.Module):
    ...
    def forward(self, field):
        ...
        # Inverse FFT
        return torch.fft.ifft2(field_fft)
...
class D2NN(nn.Module):
    def __init__(self, num_layers, layer_size, wavelength, pixel_size, 
        ...
        # Detector regions (2x5 grid, 100x100 pixels each)
        self.detector_size = layer_size // 2  # 100x100 pixels
    ...
    def forward(self, x):
        ...
        # Normalize input over spatial dimensions
        ...
        # Store intermediate fields for visualization
...
def load_datasets(batch_size=BATCH_SIZE):
    ...
    # Rotated MNIST (custom transform)
...
def visualize_fields(model, images, labels):
    model.eval()
    with torch.no_grad():
        _, fields = model(images.to(device))
